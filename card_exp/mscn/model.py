...
class SetConv(nn.Module):
    ...
    def get_features(self, samples, predicates, joins, sample_mask, predicate_mask, join_mask):
        samples = samples * sample_mask
        samples = np.sum(samples, axis=1, keepdims=False)
        sample_norm = sample_mask.sum(1, keepdims=False)
        samples =  samples / sample_norm

        predicates = predicates * predicate_mask
        predicates = np.sum(predicates, axis=1, keepdims=False)
        predicate_norm = predicate_mask.sum(1, keepdims=False)
        predicates = predicates / predicate_norm

        joins = joins * join_mask
        joins = np.sum(joins, axis=1, keepdims=False)
        join_norm = join_mask.sum(1, keepdims=False)
        joins = joins / join_norm
        out = np.concatenate((samples, predicates, joins),1)
        return out
